...
# OBS targets: SAM3 Mask and SAM3 Keypoint windows. This records perception only;
# tracker is started disabled so the robot does not move.
SIM_GUI="${SIM_GUI:-true}" \
SAM3_PROMPT="$PROMPT" \
TRACKER_START_ENABLED=false \
...
" >"$LOG_DIR/sam3_prompt_viewer.log" 2>&1 &
echo "$!" > "$PID_DIR/sam3_prompt_viewer.pid"
echo "[OK] SAM3 prompt segmentation recording stack started with prompt: $PROMPT"
